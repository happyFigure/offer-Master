...
class FrontendAgentChatApiTest(TestCase):
    ...
    def test_chat_messages_render_safe_clickable_links(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        style_source = Path("apps/web/src/styles/global.css").read_text(encoding="utf-8")

        self.assertIn("renderInlineLinks", app_source)
        self.assertIn("isSafeHttpUrl", app_source)
        self.assertIn("markdownLinkPattern", app_source)
        self.assertIn("bareUrlPattern", app_source)
        self.assertIn('target="_blank"', app_source)
        self.assertIn('rel="noreferrer"', app_source)
        self.assertIn("chat-inline-link", app_source)
        self.assertNotIn("dangerouslySetInnerHTML", app_source)
        self.assertIn(".chat-inline-link", style_source)
    ...
    def test_approval_buttons_show_busy_state_and_use_long_running_request_timeout(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        api_source = Path("apps/web/src/api/agent.ts").read_text(encoding="utf-8")
        client_source = Path("apps/web/src/api/client.ts").read_text(encoding="utf-8")

        self.assertIn("approvalWorking={workingAction?.startsWith(\"agent-approval-\") ?? false}", app_source)
        self.assertIn("approvalWorking", app_source)
        self.assertIn("正在执行", app_source)
        self.assertIn("timeoutMs", client_source)
        self.assertIn("timeoutMs: 120_000", api_source)
    ...
    def test_runtime_timeline_explains_blocked_textual_tool_calls_in_plain_chinese(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")

        self.assertIn('textual_tool_call_blocked: "疑似工具调用"', app_source)
        self.assertIn('模型把工具调用写成了普通文字', app_source)
        self.assertIn('没有当作真实工具执行', app_source)
        self.assertIn('textual_tool_call_blocked', app_source)

    def test_runtime_timeline_groups_reasoning_tools_observations_and_evidence(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        types_source = Path("apps/web/src/types/agent.ts").read_text(encoding="utf-8")

        self.assertIn('input_preview?: Record<string, unknown> | null', types_source)
        self.assertIn('result_summary?: Record<string, unknown> | null', types_source)
        self.assertIn('evidence?: Array<Record<string, unknown>> | null', types_source)
        self.assertIn('runtimeEventGroupTitle', app_source)
        self.assertIn('思考摘要', app_source)
        self.assertIn('工具调用', app_source)
        self.assertIn('观察与重试', app_source)
        self.assertIn('证据来源', app_source)
        self.assertIn('runtime-event-group', app_source)
        self.assertIn('runtime-event-detail-grid', app_source)
        self.assertIn('event.resultSummary', app_source)
        self.assertIn('event.evidence', app_source)
    ...
    def test_runtime_timeline_distinguishes_main_model_sub_agents_and_tool_returns(self) -> None:
        ...
        self.assertIn("子 Agent", app_source)
        self.assertIn("本地工具", app_source)
        self.assertIn("返回主 agent", app_source)
        self.assertIn("runtime-agent-chain", app_source)
        self.assertIn("runtime-event-payload-preview", app_source)
        self.assertIn("runtimeActorBadgeClass", app_source)
        self.assertIn("runtime-agent-chain", style_source)
        self.assertIn("runtime-event-payload-preview", style_source)
        self.assertIn("runtime-actor-badge", style_source)

    def test_runtime_timeline_has_expandable_horizontal_flow_board(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        style_source = Path("apps/web/src/styles/global.css").read_text(encoding="utf-8")

        self.assertIn("runtimeFlowExpanded", app_source)
        self.assertIn("setRuntimeFlowExpanded", app_source)
        self.assertIn('aria-expanded={runtimeFlowExpanded}', app_source)
        self.assertIn("展开流程图", app_source)
        self.assertIn("收起流程图", app_source)
        self.assertIn("RuntimeExpandedFlowBoard", app_source)
        self.assertIn("runtime-flow-toggle", app_source)
        self.assertIn("runtime-flow-expanded-board", app_source)
        self.assertIn("runtime-flow-lane-list", app_source)
        self.assertIn("runtime-flow-lane-card", style_source)
        self.assertIn("overflow-x: auto;", _css_block(style_source, ".runtime-flow-expanded-board"))

    def test_runtime_timeline_uses_distinct_actor_card_identities(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        style_source = Path("apps/web/src/styles/global.css").read_text(encoding="utf-8")

        self.assertIn("runtimeActorVisual", app_source)
        self.assertIn("主模型调度", app_source)
        self.assertIn("子 Agent 执行", app_source)
        self.assertIn("工具函数执行", app_source)
        self.assertIn("结果返回主 agent", app_source)
        self.assertIn("runtime-flow-lane-card-main", style_source)
        self.assertIn("runtime-flow-lane-card-subagent", style_source)
        self.assertIn("runtime-flow-lane-card-tool", style_source)
        self.assertIn("runtime-flow-lane-card-return", style_source)
        self.assertIn("runtime-event-card-main", style_source)
        self.assertIn("runtime-event-card-subagent", style_source)
        self.assertIn("runtime-event-card-tool", style_source)
        self.assertIn("runtime-event-card-return", style_source)

    def test_runtime_timeline_staggers_new_event_cards(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")

        self.assertIn("runtimeEventAnimationDelay", app_source)
        self.assertIn("animationDelay: runtimeEventAnimationDelay(index)", app_source)
        self.assertIn("runtime-event-animated", app_source)
        self.assertIn("Math.min(index, 8) * 35", app_source)

    def test_runtime_timeline_auto_scrolls_only_when_user_tracks_latest_event(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")

        self.assertIn("runtimeTimelineRef", app_source)
        self.assertIn("shouldRuntimeTimelineStickToBottomRef", app_source)
        self.assertIn("handleRuntimeTimelineScroll", app_source)
        self.assertIn("onScroll={handleRuntimeTimelineScroll}", app_source)
        self.assertIn("scrollElementToBottom(runtimeTimelineRef.current)", app_source)
        self.assertIn("isScrollableElementNearBottom(element)", app_source)
        self.assertIn("[visibleEvents.length, latestEvent?.id, isWorking]", app_source)
        self.assertIn("shouldRuntimeTimelineStickToBottomRef.current = isScrollableElementNearBottom(element)", app_source)
    ...
    def test_chat_side_panel_displays_agent_task_plan_stages_and_handoffs(self) -> None:
        app_source = Path("apps/web/src/app/App.tsx").read_text(encoding="utf-8")
        api_source = Path("apps/web/src/api/agent.ts").read_text(encoding="utf-8")
        types_source = Path("apps/web/src/types/agent.ts").read_text(encoding="utf-8")
        style_source = Path("apps/web/src/styles/global.css").read_text(encoding="utf-8")

        self.assertIn("export interface AgentTaskPlanStage", types_source)
        self.assertIn("business_action?: string | null", types_source)
        self.assertIn("allowed_capabilities: string[]", types_source)
        self.assertIn("tool_strategy: Record<string, unknown>", types_source)
        self.assertIn("ranking_policy: string[]", types_source)
        self.assertIn("received_context", types_source)
        self.assertIn("handoff_payload", types_source)
        self.assertIn("export async function getAgentTaskPlan", api_source)
        self.assertIn("`/api/v1/agent/tasks/${taskId}/plan`", api_source)
        self.assertIn("taskPlan", app_source)
        self.assertIn("refreshActiveTaskPlan", app_source)
        self.assertIn("ChatTaskPlanPanel", app_source)
        self.assertIn("阶段任务链", app_source)
        self.assertIn("业务动作", app_source)
        self.assertIn("可用工具", app_source)
        self.assertIn("工具策略", app_source)
        self.assertIn("匹配排序规则", app_source)
        self.assertIn("阶段产物", app_source)
        self.assertIn("收到上游信息", app_source)
        self.assertIn("chat-task-plan-panel", style_source)
        self.assertIn("task-plan-step-card", style_source)
        self.assertIn("task-plan-strategy-box", style_source)
        self.assertIn("task-plan-ranking-list", style_source)
        self.assertIn("task-plan-handoff-box", style_source)
